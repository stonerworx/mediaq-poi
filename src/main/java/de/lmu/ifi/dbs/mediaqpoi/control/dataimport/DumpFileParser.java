	...
	private void parseVideoMetaData(String fileContent) throws Exception {
		try {
			...
			for (String row : insertRows) {
				...
				TrajectoryPoint point = new TrajectoryPoint(Integer.parseInt(frame), Double.parseDouble(latitude), Double.parseDouble(longitude), Double.parseDouble(thetaX),
						Double.parseDouble(thetaY), Double.parseDouble(thetaZ), Double.parseDouble(r), Integer.parseInt(alpha), Long.parseLong(timeCode));
