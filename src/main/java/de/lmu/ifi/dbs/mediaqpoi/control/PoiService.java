...
public class PoiService implements IPoiService {
	...
	public List<Poi> getNearbyPois(Video video) {
		...
		if (nearbyPois == null) {
			...
			PlacesList places = GooglePlacesApi.searchPlaces(video
					.getTrajectory().calculateCenter(), video.getTrajectory()
					.calculateSearchRange());

			for (Place place : places.results) {
				Poi poi = new Poi(place.id, place.reference,
						place.geometry.location.getLatitude(),
	...
	public List<Poi> getVisiblePois(Video video) {
		...
		Map<Integer, Map<String, List>> timeline = (HashMap<Integer, Map<String, List>>) syncCache
				.get(timelineKey);
		...
		if (visiblePois == null || timeline == null) {
			...
			for (Long timeStamp : trajectory.getTimeStampedPoints().keySet()) {
				...
				// DATA AT THE MOMENT.
				// --- DEMO DATA
				if (trajectoryPoint.getFrame() > 20
						&& trajectoryPoint.getFrame() < 50) {
					Poi poi = pois.get(0);
				...
				if (trajectoryPoint.getFrame() > 30
						&& trajectoryPoint.getFrame() < 40) {
					Poi poi = pois.get(1);
					if (!visible.contains(poi)) {
						visible.add(poi);
					}
